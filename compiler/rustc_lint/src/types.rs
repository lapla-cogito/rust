...
fn get_nullable_type<'tcx>(
    ...
    Some(match *ty.kind() {
        ...
        ty::Int(_) | ty::Uint(_) | ty::Char | ty::Bool | ty::RawPtr(..) => ty,
        ...
        ty::Adt(ty_def, args) => {
            ...
            if let BackendRepr::Scalar(field_ty_scalar) = field_ty_abi {
                match field_ty_scalar.valid_range(&tcx) {
                    ...
                    {
                        return Some(get_nullable_type(tcx, typing_env, field_ty).unwrap_or_else(
                            || {
                                bug!(
                                    "improper_ctypes: no nullable form for nonnull scalar {field_ty:?}"
                                )
                            },
                        ));
                    }
